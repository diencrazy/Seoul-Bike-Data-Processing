...
---
\centering
\includegraphics[width=10cm]{D:/TH_MHTKKTNC/logo.jpg}
\raggedright
\clearpage
\tableofcontents
...
library(rio)
raw_data = rio::import("D:/TH_MHTKKTNC/TH/SeoulBikeData.csv")|> 
  janitor::clean_names()
...
data <- raw_data |> 
  ...
  mutate(year = factor(year, ordered = TRUE, 
                       levels = c("2017", "2018")),  
                        seasons = factor(seasons, ordered = TRUE, 
...
\subsubsection{Lọc dữ liệu bằng giá trị chuẩn hóa và khoảng cách Mahalanobis}
...
\textbf{Nhận xét:} Số quan trắc bị loại bỏ xấp xỉ 5% bộ dữ liệu nằm ở mức chấp nhận được (dưới 10%).
...
Bỏ biến functioning\_day ra khỏi mô hình trên và kiểm tra đa cộng tuyến:
...
\subsection{Ngày lễ Halowen}

\textbf{Dự đoán lượng xe thuê vào 16 giờ ngày Halowen}
...
tmp<- subset(cleaned_data, day==31& month==10& year==2018)
bike_251217 <- tmp[17,] 
actual_count_251217 <- bike_251217$rented_bike_count
# 25-12-2017
gam_breakdown <- predict_parts(explainer = explainer_gam, new_observation = bike_251217)
# Tạo một bảng dữ liệu với giá trị thực tế và giá trị dự đoán
breakdown_with_actual <- gam_breakdown |> 
  cbind(data.frame(
    variable = "Observed Value",
    contribution = actual_count_251217
  ))

# Hiển thị bảng phân tích của mô hình GAM với giá trị thực tế
...
\textbf{Dự đoán trung bình lượng xe được thuê trong ngày Halowen:}

```{r}
newdata = subset(cleaned_data, day==31& month==10& year==2018)
newdata_1= predict(gam.model, newdata = newdata,
                        type = "response")

sum(newdata_1)
sum(newdata$rented_bike_count)
```

\textbf{Nhận xét:} Kết cho ta giá trị dự đoán trung bình lượng xe được thuê trong ngày Halowen là 22226.04 và giá trị thực tế là 21545, giá trị dự đoán cao hơn thực tế nhiều (Khoảng 680 xe)
...
```{r echo=FALSE, out.width = "120%"}
 split <- sample.split(reg_data$rented_bike_count,SplitRatio=0.7)
 train_set <- subset(reg_data,split==TRUE)
